...
class BusStopFormSet(BaseInlineFormSet):
    ...
    def clean(self):
        ...
        if self._shift_has_overlap(self.instance.bus.shifts, departure_time, arrival_time):
            raise ValidationError("This bus already has a conflicting shift.")
        if self._shift_has_overlap(self.instance.driver.shifts, departure_time, arrival_time):
